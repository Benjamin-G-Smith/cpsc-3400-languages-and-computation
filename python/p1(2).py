...
def run():
    ...
    userInput = raw_input("Would you like to query for a string ['y'/'n']: ").lower()
    return userInput == 'y'
...
'''docs it the path to open the file directory'''

directory = raw_input("Enter Your Directory Path: ")
...
while(done):
    ...
    if(searchSrt != '\n'):
        ...
        topValues = sorted(values.iteritems(), key=lambda x:-x[1])[:3]
